...
import { CancellationToken, DebugSession, Uri, Position, debug, workspace } from 'vscode';
import { callDebugFunction, getVariablesRecursive } from './typed-debug';
import { logDebug, logWarn } from '../log';

export async function getValueWithLookupMethod(
    uri: Uri,
    line: number,
    column: number,
    frameId: number,
    ...
    const variableInfo = await inspectVariableAtPosition(
        debugSession,
        uri,
...
async function inspectVariableAtPosition(
    ...
    const stackTraceResponse = await callDebugFunction('stackTrace', {
